...
Interface Vision is a visual object language and fully [composable](http://en.wikipedia.org/wiki/Composability) object framework. Instead of developing software using [source code](http://en.wikipedia.org/wiki/Source_code), programmers develop by visually connecting parts [(1)](#id-1) together.
...
We're creating Interface Vision using a [programming paradigm](http://en.wikipedia.org/wiki/Programming_paradigm), we developed in-house, called Simple Interface Programming (SIP).
...
SIP doesn't allow developers to create subroutines (methods, functions, modules, etc.) which take <a href="http://en.wikipedia.org/wiki/Parameter_(computer_programming)">parameters</a>.
...
Traditionally, <a href="http://en.wikipedia.org/wiki/Property_(programming)">properties</a> and <a href="http://en.wikipedia.org/wiki/Attribute_(computing)">attributes</a> store information internal to a part. External information is provided to parts through parameters.
...
We store and load all of these attached parts in a configuration file using [serialization](http://en.wikipediadia.org/wiki/Serialization). An example of just such a configuration is shown in [Source-1.5](#id-s1-5) and [Source-1.7](#id-s1-7).

It is also possible to initialize parts using source code as shown in [Source-1.3](#id-s1-3).
...
###### Source-1.2: A Long and Int part.
...
The example code in [Source-1.4](#id-s1-4) is almost boilerplate except the global Part called Ccm (which stands for [Composite Centric Memory (CCM)](#id-composite-centric-memory) ).
...
What is interesting about the parts we have created is that they all have similar <a href="http://en.wikipedia.org/wiki/Interface_(computing)">interfaces</a>: they all look the same to an external observer. However, internally, these parts are doing completely different things.
...
Since the interface is the exact same between Long, Int and FormValue, Add is fully unaware of where the values are coming from. This means that the part which Adds is not “connected” to the part that finds the value on the form: they are [decoupled](http://en.wikipedia.org/wiki/Decoupling) [7](#id-7).
...
Using traditional [programming paradigms](http://en.wikipedia.org/wiki/Programming_paradigm), you end up with thousands of objects: each with their own unique [method signature](http://en.wikipedia.org/wiki/Method_signature)/<a href="http://en.wikipedia.org/wiki/Interface_(computing)">interface</a>. With SIP, you end up with thousands of Parts with the same signature/interface.
...
What is really interesting about this json is that it looks very similar to the example C# in [Source-1.5](#id-s1-5). In fact, there is an almost one-to-one relationship between the persisted version a program and the program written using C# object initializers.
...
* **Locators** - Some parts are created specifically to locate information within data structures [12](#id-12). An example of just such a locator is the FormValue type defined in [Source-1.4](#id-s1-4). Every data structure must have at least one Part defined that can search that data structure.
...
###### Source-1.6: A Mutex Part. {#id-s1-6}
...
###### Source-1.7: Add configured with a mutex. {#id-s1-7}
...
### The Greatest Disadvantage of Traditional Programming Paradigms

The greatest disadvantage of traditional [programming paradigms](http://en.wikipedia.org/wiki/Programming_paradigm), which allow parameters, is that communication between parts can't be standardized. Every method or function created by a developer leads to greater specialization of the program’s communication system and the source code.

We can’t help it. This is an intrinsic aspect of traditional programming paradigms. The more parts we have, the more complex the source code becomes.

However, with SIP, every part looks the exact same. The complexity of our programs do not go up as we add new types of parts.

## Footnotes

{#id-1} 1. We look at parts as being "similar" to systems, objects, classes, modules, functions, subroutines and the such.

{#id-2} 2. Our example code is written in C#.
